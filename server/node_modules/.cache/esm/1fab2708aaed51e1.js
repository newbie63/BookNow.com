let mongoose,bcrypt;_aae‍.x([["default",()=>_aae‍.o]]);_aae‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);_aae‍.w("bcrypt",[["default",["bcrypt"],function(v){bcrypt=v}]]);
...
userSchema.pre("save", function (next) {
  ...
  if (user.isModified("password")) {
    return bcrypt.hash(user.password, 12, function (err, hash) {
      if (err) {
        _aae‍.g.console.log("BCRYPT HASH ERR ", err);
        return next(err);
      }
      user.password = hash;
      return next();
...
userSchema.methods.comparePassword = function(password,next){
  bcrypt.compare(password,this.password,function(err,match){
    if(err){
      _aae‍.g.console.log("Incorrect password",err);
      return next(err,false);
    }

    _aae‍.g.console.log("Password match",match);
    return next(null,match);
  });
};

_aae‍.d(mongoose.model("User", userSchema));
