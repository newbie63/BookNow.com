...
export const createConnectAccount = async (req, res) => {
  ...
  res.send(link);
  //     const link = 'https://dashboard.stripe.com/login';
  //     res.send(link);
...
export const stripeSessionId = async (req, res) => {
  ...
  //await User.findByIdAndUpdate(req.user._id, { Coins: coins }).exec();
  //console.log("User updated info:",req.user);
  //console.log("User updated info:",req.user._id.Coins);
...
export const stripeSuccess = async (req, res) => {
  try {
    ...
    if (session.payment_status === "paid") {
      const item = await Hotel.findById(hotelId).populate("postedBy").exec();
      const fee = (item.price * 20)/100 ;
      const coins = user.Coins+(fee*10)/100;
      //console.log(coins);
